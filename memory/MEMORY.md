...
- [Kcalix Connector — índice operacional](kcalix-connector/README.md) — Fase 00 / `KCX-CONN-005` e `KCX-CONN-000` concluídas e validadas no aparelho em 2026-07-22. A `KCX-CONN-001` consolidou cardio sem redigitação, musculação enriquecida e body fat BIA como os três casos do piloto; passos, sono e FC de repouso foram adiados. O sample oficial `android/health-samples/HealthConnectSample` está pinado no commit `47f0144f6e994f7831a41499843a0f6a9d87cb75`. Em 2026-08-08, a [revisão oficial](kcalix-connector/evidence/KCX-CONN-001-api-review-2026-08-08.md) aprovou stable `connect-client:1.1.0`, seis permissões de leitura, janela de 7 dias, paginação completa, [contrato e schema](kcalix-connector/contracts/kcx-health-observation-1.md) e fixtures sintéticas. A spec está `APPROVED` e a Issue `DONE` após testes positivo, negativo, privacidade e permissões; nenhuma integração Kotlin ou leitura real foi liberada. A aprovação técnica pertence ao agente/revisor, não ao usuário; o usuário recebe somente escolhas reais de produto e instruções de testes físicos. A próxima Issue `READY` é `KCX-CONN-002` (PRD); leitura/export real pertence à `KCX-CONN-007`, após ADR, privacidade e permissões. `KCX-CONN-021` continua como polish não bloqueante.
- Protocolo de sessão do Connector atualizado em 2026-08-08: `/start` roteia por branch/Issue;
  o encerramento atualiza Issue, spec, evidências, roadmap, índice, memória e log; aprovação de
  engenharia fica com o agente; commit/push/deploy/APK só ocorrem por solicitação explícita.
  Retomada exata em [2026-08-08-KCX-CONN-001-api-contract.md](kcalix-connector/sessions/2026-08-08-KCX-CONN-001-api-contract.md).
