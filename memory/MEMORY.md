...
- [Kcalix Connector — índice operacional](kcalix-connector/README.md) — Fase 00 / `KCX-CONN-005` e `KCX-CONN-000` concluídas e validadas no aparelho em 2026-07-22. A descoberta de produto de 2026-07-23 priorizou cardio sem redigitação, musculação enriquecida e body fat BIA. A `KCX-CONN-001` agora possui [spec de observação](kcalix-connector/specs/KCX-CONN-001.md) e [roteiro de revisão especializada](kcalix-connector/reviews/KCX-CONN-001-reference-project-review.md) em `DRAFT`: o projeto de referência será confrontado com a API oficial antes de qualquer código; a leitura/export real pertence à `KCX-CONN-007`, após PRD, ADR, privacidade e permissões. `KCX-CONN-021` continua como polish não bloqueante.
