...
- [Kcalix Connector — índice operacional](kcalix-connector/README.md) — Fase 00 concluída e validada no aparelho. `KCX-CONN-001` aprovou stable `connect-client:1.1.0`, seis permissões de leitura, janela de 7 dias e o contrato `kcx-health-observation/1`. Em 2026-08-08, a `KCX-CONN-002` aprovou o [PRD do piloto](kcalix-connector/PRD.md), versão `kcx-connector-prd/1`, fechando G1 — Valor. O piloto mantém exatamente cardio sem redigitação, musculação enriquecida e body fat BIA; define baseline manual de 7 dias, período de 14–28 dias, política de zonas sem estimativa silenciosa por idade, métricas e decisão `GO | ADJUST | NO-GO`. Passos, sono e FC de repouso continuam adiados. A próxima Issue `READY` é `KCX-CONN-003` (ADR Kotlin/Compose); leitura real pertence à `KCX-CONN-007`, após arquitetura, privacidade e permissões. Nenhuma integração Kotlin ou leitura real foi liberada. `KCX-CONN-021` continua como polish não bloqueante.
- Protocolo de sessão do Connector atualizado em 2026-08-08: `/start` roteia por branch/Issue;
  ...
  Retomada exata em [2026-08-08-KCX-CONN-002-prd.md](kcalix-connector/sessions/2026-08-08-KCX-CONN-002-prd.md).
