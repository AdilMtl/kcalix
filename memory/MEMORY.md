...
- [Kcalix Connector — índice operacional](kcalix-connector/README.md) — Fase 00 / `KCX-CONN-005` e `KCX-CONN-000` concluídas e validadas no aparelho em 2026-07-22. Em 2026-07-27, a retomada da `KCX-CONN-001` consolidou cardio sem redigitação, musculação enriquecida e body fat BIA como os três casos do piloto; passos, sono e FC de repouso foram adiados. A [spec de observação](kcalix-connector/specs/KCX-CONN-001.md) e o [roteiro de revisão especializada](kcalix-connector/reviews/KCX-CONN-001-reference-project-review.md) permanecem em `DRAFT`: falta registrar a referência técnica, executar a revisão e aprovar `kcx-health-observation/1`. A leitura/export real pertence à `KCX-CONN-007`, após PRD, ADR, privacidade e permissões. `KCX-CONN-003` agora ratifica a base Kotlin/Compose; `KCX-CONN-021` continua como polish não bloqueante.
